...
@section('title', 'Berkas Dikembalikan - SIRPL')
...
@section('content')
    ...
    <p style="font-size: 14px; color: #434655; line-height: 1.5; margin: 0 auto 24px auto; text-align: center; max-width: 500px;">
        Setelah proses verifikasi oleh Admin Program Studi, beberapa dokumen pada pendaftaran Anda <strong style="color: #ba1a1a;">memerlukan perbaikan</strong>.
    </p>

    <!-- Error Alert Section -->
    <div style="background-color: #ffdad6; border-radius: 12px; padding: 24px; margin-bottom: 24px; text-align: left;">
        <h3 style="color: #93000a; margin: 0 0 12px 0; font-size: 16px; font-weight: 600;">Catatan dari Admin:</h3>
        <p style="color: #93000a; font-size: 14px; line-height: 1.6; margin: 0;">
            {{ $catatan ?? 'Silakan cek detail di dashboard Anda untuk melihat dokumen mana yang perlu diperbaiki.' }}
        </p>
    </div>

    <!-- Steps Section -->
    <div style="background-color: #f2f3ff; border-radius: 12px; padding: 24px; margin-bottom: 32px; text-align: left;">
        <h3 style="font-size: 16px; color: #004ac6; margin: 0 0 16px 0; font-weight: 600;">Yang Perlu Anda Lakukan:</h3>
        <table role="presentation" cellspacing="0" cellpadding="0" style="width: 100%; border-collapse: collapse;">
            @php
                $steps = [
                    'Masuk ke akun SIRPL Anda.',
                    'Periksa catatan verifikasi pada setiap dokumen.',
                    'Perbaiki dan unggah ulang dokumen yang diperlukan.',
                    'Kirim kembali pendaftaran Anda.'
                ];
            @endphp
