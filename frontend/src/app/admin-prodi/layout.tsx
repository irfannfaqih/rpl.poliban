  ...
  return (
    <header className="sticky top-0 z-30 flex h-14 items-center justify-between border-b bg-background/80 backdrop-blur-md px-6 shadow-sm">
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs font-bold uppercase tracking-widest text-primary">Portal Admin Prodi</span>
      </div>
...
const sidebarLinks = [
  ...
  { href: "/admin-prodi/kurikulum", label: "Kurikulum & Matriks", icon: BookOpen },
  { href: "/admin-prodi/jadwal", label: "Kalender Jadwal", icon: CalendarDays },
  { href: "/admin-prodi/matriks", label: "Review Matriks", icon: ArrowRightLeft },
      ...
      <div className="border-t border-sidebar-border p-4 bg-muted/5">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 overflow-hidden">
            <p className="truncate text-sm font-medium">{user?.nama || "Admin Prodi"}</p>
            <p className="truncate text-xs text-muted-foreground">{user?.prodi?.nama || 'Admin Prodi'}</p>
          </div>
        </div>
      </div>
    </div>
