...
const statusLabel: Record<string, string> = {
  menunggu_approval_kaprodi: "Menunggu Approval Kaprodi",
...
function statusClass(status: string) {
  if (status === "approved_final" || status === "menunggu_approval_pimpinan") {
    return "bg-emerald-50 text-emerald-700 border-emerald-200";
  }
  if (status?.startsWith("ditolak")) return "bg-red-50 text-red-700 border-red-200";
...
export default function KaprodiPlenoApprovalPage() {
  ...
  const { data: approvals = [], isLoading, error } = useQuery({
    queryKey: ["kaprodi-pleno-approvals", statusFilter],
    queryFn: async () => {
      const params: Record<string, string> = {};
      if (statusFilter !== "all") params.status = statusFilter;
      const { data } = await api.get("/kaprodi/pleno-approvals", { params });
      return data.data || [];
    },
  });
  ...
  const rejectMutation = useMutation({
    mutationFn: () => api.post(`/kaprodi/pleno-approvals/${rejectId}/reject`, { catatan: rejectNote }),
    onSuccess: () => {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-8 pb-20">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-foreground">Approval Pleno Kaprodi</h1>
          <p className="mt-1 text-xs text-muted-foreground">
            Verifikasi hasil pleno prodi sebelum diteruskan ke Pimpinan/Direktur.
          </p>
        </div>
        <Select value={statusFilter} onValueChange={(val) => setStatusFilter(val || "all")}>
          ...
          <SelectContent>
            ...
            <SelectItem value="menunggu_approval_kaprodi">Menunggu Kaprodi</SelectItem>
            <SelectItem value="ditolak_kaprodi">Ditolak Kaprodi</SelectItem>
            <SelectItem value="menunggu_approval_pimpinan">Diteruskan ke Pimpinan</SelectItem>
            ...
            <div key={approval.id} className="rounded-2xl border bg-card p-5 shadow-sm">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Diajukan oleh {approval.submitter?.nama || "Admin Prodi"} pada{" "}
                    {approval.submitted_at ? new Date(approval.submitted_at).toLocaleString("id-ID") : "-"}
                  </p>
                </div>
