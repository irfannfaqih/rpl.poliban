  ...
  return (
    <header className="sticky top-0 z-30 flex h-14 items-center justify-between border-b bg-background/80 backdrop-blur-md px-6 shadow-sm">
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs font-bold uppercase tracking-widest text-primary">Portal IT & Pengelola Sistem</span>
      </div>
...
const sidebarLinks = [
  { href: "/super-admin/gelombang", label: "Manajemen Gelombang", icon: CalendarDays },
      ...
      <div className="border-t border-sidebar-border p-4 bg-muted/5">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 overflow-hidden">
            <p className="truncate text-sm font-medium">{adminInfo?.name || "System Admin"}</p>
            <p className="truncate text-xs text-muted-foreground">{adminInfo?.role}</p>
          </div>
        </div>
      </div>
    </div>
